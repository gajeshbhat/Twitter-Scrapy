...
class TweetScraper(CrawlSpider):
    ...
    def __init__(self, query='',lang='',country='',top_tweet=False):
        ...
        self.city_name = str(query.strip('#'))
    ...
    def parse_tweet_item(self, items):
        for item in items:
            try:
                ...
                # Encode city in item
                tweet['hash_tag'] = self.city_name
                tweet['city_country'] = self.country_name
                ...
                tweet['user_profile_name'] = item.xpath('.//@data-name').extract()[0]
                tweet['user_screen_name'] = item.xpath('.//@data-screen-name').extract()[0]
                tweet['user_avatar'] = item.xpath('.//div[@class="content"]/div[@class="stream-item-header"]/a/img/@src').extract()[0]
